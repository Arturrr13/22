...
function YsNo() {
  scrollDown();
  $(".chat-content-list").append(
    '<div class="chat-content-buttons-gender"><div class="chat-content-buttons-gender-block"><span class="chooseGenderM" id="yeas">Дa</span></div><div class="chat-content-buttons-gender-block"><span class="chooseGenderW" id="no">Нет</span></div></div>'
  );
  $("#yeas").click(() => {
...
function showForm() {
  $(".iframe-form").addClass("show"),
    ...
    const scrollToFrom = ($('.content').scrollTop() + 400)
    $('.content').animate({scrollTop: scrollToFrom}, 800)
    $('.glow-button').css('opacity', '1');
}
...
const scrollBtn = document.getElementById("scroll_id")
const showMoreBtn = document.getElementById("glow-button")
let showComment = false

form.addEventListener("submit", function(e){
  e.preventDefault();
  if(inputName.value.length > 1 && inputPhone.value.length > 12 && /^\+380\d*$/.test(inputPhone.value)){
    const name = document.getElementById( "input-name").value
    const phone = document.getElementById( "input-phone").value
    const formDate = ` Стать: ${gender};\n Дата рождения: ${bornData};\n Наличие аллергий: ${diseases};\n Имя: ${name};\n Телефон: ${phone} `
    inputName.classList.remove('inputErr')
    inputPhone.classList.remove('inputErr') 

    console.log(formDate)
  } else if(inputName.value.length <= 1){
    inputName.classList.add('inputErr')
    inputPhone.value.length > 12 ? inputPhone.classList.remove('inputErr') : null
  } else if(inputPhone.value.length <= 12 || !/^\+380\d*$/.test(inputPhone.value)){
    inputPhone.classList.add('inputErr')
    inputName.value.length > 1 ? inputName.classList.remove('inputErr') : null
  }
});

inputPhone.addEventListener('focus', () => !/^\+380\d*$/.test(inputPhone.value) ? inputPhone.value = '+380' : null );

inputPhone.addEventListener('keydown', e => {
  !/\d/.test(e.key) && e.key !== 'Backspace' ? e.preventDefault() : null
  if(e.key === '+' && inputPhone.value.length < 1){
    e.preventDefault()
    inputPhone.value = '+380'
  }
  if (e.key === 'Backspace' && inputPhone.value.length <= 4) {
    e.preventDefault();
    inputPhone.value = '+380'
  }
});

scrollBtn.addEventListener('click', () => $('.content').animate({scrollTop:$('.content').prop("scrollHeight")}, 800))

showMoreBtn.addEventListener('click', () => {
  if(showComment){
    showMoreBtn.innerHTML = 'Показать еще'
    document.querySelectorAll('.comment-item').forEach((el, index) => {
      if(index > 2){
        el.classList.remove('comment-item-show')
      }
    })
    showComment = false
  } else {
    ...
    document.querySelectorAll('.comment-item').forEach((el) => {
      el.classList.add('comment-item-show')
    })
    showComment = true
  }
})
